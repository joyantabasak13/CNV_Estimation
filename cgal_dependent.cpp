...
void print_cntg_length()
{
    ...
    cout << mark << " th"
         << " contig length is maximum. Max length is " << max_length << endl;
}
void updateGCArr()
{
...
double computeLikelihood(char *file)
{
    ...
    for (it1 = readsInContig.begin(); it1 != readsInContig.end(); it1++)
    { ///iterates over every contig and its set of reads
...
double computeLikelihoodForCNVdependent(int copyCount, double prevLikelihoodFinal, int contigNo)
{
    ...
    double prevEffective, newEffective;
    for (int i = 0; i < MAX_INSERTSIZE_CNV; i++)
    { ///i=insert length
        double extraLenforCNV = 0;
        if (insertLenCount[i] != 0)
        { ///not equal to 0 means this insert length has some count
    ...
    { ///iterates over every contig and its set of reads
        ...
        { ///basay change(mim)
            ...
            for (it2 = it1->second.begin(); it2 != it1->second.end(); it2++)
            {
                ...
                double sum1 = 0.0;
                for (int j = 0; j < sz; j++)
                { ///this iteration is for new prob calculation
                    if (multiMapReads[*it2][j].contigNo == contigNo)
                    {
                        sum1 += multiMapReads[*it2][j].prob;
                    }
...
void calculateGcVsWeight()
{
    ...
    while (1)
    {
        ...
        while (curr_pos < pos)
        { ///we haven't reached the file given read starting pos
            if (pq.top() >= curr_pos)
            {
                if (pq.empty() == false)
                {
        ...
        {
            //joyanta
            //cout<< "Current Contig "<< curr_contig << "MP length "<< sizeof(mp) <<endl;
            prev_pos = curr_pos;
    ...
    //joyanta
    // contigLengths.size() 1089
    // gcContentArr size: Row 1 Column 3
    // mp Size: Row 2 Column 3
    // gcVSweight Size : 24
    // ContigSIze of 0 th contig is 34902
    // After for loop
    // ContigSIze of 1 th contig is 541
    ...
    for (int i = 0; i < contigLengths.size(); i++)
    {
        ...
        for (int j = 0; j < contigSize; j++)
        {
            ...
            gcVSweight[gcValue] += mp[i][j]; //here is the problem
